  ...
  HR.MainFrame:SetClampedToScreen(true);
    ...
    function HR.MainFrame:ToggleLock ()
      if HeroRotationDB.Locked then
        HR.MainFrame:Unlock ();
        HR.Print("HappyRotation UI is now |cff00ff00unlocked|r.");
      else
        HR.MainFrame:Lock ();
        HR.Print("HappyRotation UI is now |cffff0000locked|r.");
      end
    end
    -- Start Move
...
--- ======= MAIN =======
  ...
  function HR.Pulse ()
	if not Settings.General.AdaptivePerformance then
		TickRate = Settings.General.TickRate
	else
		local CurrentFPS = GetFramerate()
		if GetTime() > TickChangeDelay + 2 then
			if Settings.General.TargetFrames - CurrentFPS >= 0 then
				if TickRate + Settings.General.TargetFrames - CurrentFPS >= 250 then
					TickRate = 250
					TickChangeDelay = GetTime()
				else 
					TickRate = TickRate + Settings.General.TargetFrames - CurrentFPS
					TickChangeDelay = GetTime()
				end
			else
				if TickRate + Settings.General.TargetFrames - CurrentFPS <= 0 then
					TickRate = 0
					TickChangeDelay = GetTime()
				else 
					TickRate = TickRate + Settings.General.TargetFrames - CurrentFPS
    ...
    if GetTime() > HR.Timer.Pulse and HR.Locked() then
      HR.Timer.Pulse = GetTime() + 0.066 + TickRate/1000;
